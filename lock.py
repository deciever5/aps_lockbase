class Lock:
    ...
    """def __str__(self):
        return f'Number: {self._number}, Lock Type: {self._lock_type}, Length: {self._length}, Finish: {self._finish}, ' \
               f'Profile: {self._profile}, Quantity: {self._quantity}, Body Pins: {self._body_pins}, Side Pins: {self._side_pins}, Extension Pins: {self._ext_pin} '"""
...
def create_locklist(df):
    ...
    for i in range(df.shape[0]):
        ...
        ext_pin = pinning[1:] if len(pinning) > 0 else None
        pin_sum = None
