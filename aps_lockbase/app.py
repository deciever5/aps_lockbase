...
def upload_files():
    ...
    text, html_table = models.extract_txt_from_pdf(app, pdf_filename)
    df = models.create_df_from_csv(app, csv_filename)

    return render_template('table.html', text=text, csv_data=df.to_html(max_rows=30, header=True),
                           csv_filename=csv_filename, pdf_data=html_table, pdf_filename=pdf_filename)
